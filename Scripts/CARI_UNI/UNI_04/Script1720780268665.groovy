...
//	if (entry != 'Akademi Farmasi Mitra Sehat Mandiri Sidoarjo') {
...
// Optionally, store names and web_pages as pairs in a list of maps
def list = []
jsonArray.each { entry ->
	
	if (entry == 'Universitas Semarang') 
		{
			println("Skipping ${entry}...")
			return
		}
	else {
		list << ["name": entry.name, "web_page": entry.web_pages[0]]
		}	
